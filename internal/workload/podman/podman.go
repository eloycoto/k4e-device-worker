...
func (p *podman) Logs(podID string, res io.Writer) (context.CancelFunc, error) {
	...
	writers := map[string]*bufio.Reader{}
	for _, container := range podInfo.Containers {
		res := &bytes.Buffer{}
		...
		writers[container.ID] = bufio.NewReader(res)
	}

	go func() {
		for {
			for containerID, buffer := range writers {
				resultLine := []byte{}
				for {
...
func (p *podman) containerLog(ctx context.Context, containerID string, res io.Writer) error {
	...
	go containers.Logs(ctx, containerID, &opts, stdoutCh, stderrCh)
